...
export function ProfileSkillsList({ skills, isOwner }) {
  ...
  return (
    <div className="space-y-6">
      {/* Title section */}
      <div className="flex flex-col gap-1 border-b border-[var(--gray-200)] pb-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
        <div className="flex rounded-lg border border-[var(--gray-400)] bg-[var(--background-100)] p-1 font-medium">
          {[
            { id: 'all', label: 'All' },
      ...
      <div className="space-y-4">
        {displayedSkills.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 bg-[var(--background-100)] border-[var(--gray-400)] shadow-card rounded-lg text-center">
            <span className="font-mono text-xs text-[var(--gray-700)]/50 mb-2">$ no published skills</span>
            <p className="text-xs text-[var(--gray-700)]">Publish a skill to see it listed here.</p>
          </div>
        ) : (
          displayedSkills.map((skill) => {
            ...
            return (
              <div 
                key={skill.id}
                className="bg-[var(--background-100)] border-[var(--gray-400)] shadow-card rounded-xl p-5 hover:border-white/20 hover:bg-[var(--gray-100)] transition-all duration-200"
              >
                <div className="flex flex-col lg:grid lg:grid-cols-12 gap-5 items-start lg:items-center">
                  ...
                  <div className="lg:col-span-2 flex items-center gap-1.5 text-xs text-[var(--primary)] lg:justify-center w-full lg:w-auto border-t lg:border-t-0 border-[var(--gray-200)] pt-3 lg:pt-0">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <CheckCircle2 className="w-3.5 h-3.5 text-[var(--blue-700)] fill-kresh-green/10" />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center">
                    {isOwner && (
                      <Link 
                        ...
                        className="p-1.5 rounded hover:bg-[var(--gray-200)] text-[var(--gray-700)] hover:text-[var(--primary)] transition-colors flex items-center gap-1 text-[11px]"
      ...
      {!isExpanded && hiddenCount > 0 && (
        <div className="flex items-center justify-center pt-2">
          <Button 
            ...
            className="text-xs py-2 px-6 flex items-center gap-1.5 rounded-lg border-[var(--gray-200)] hover:bg-[var(--gray-100)] cursor-pointer"
          >
            <span>View more skills ({hiddenCount})</span>
            <ChevronDown className="w-4 h-4 text-[var(--gray-700)]" />
          </Button>
        </div>
      )}
      ...
      {isExpanded && sortedSkills.length > initialLimit && (
        <div className="flex items-center justify-center pt-2">
          ...
          >
            <span>Show less</span>
            <ChevronUp className="w-4 h-4 text-[var(--gray-700)]" />
          </Button>
        </div>
      )}
